...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Restaurants
      .findOne(_.pick(params, _.keys(Restaurants.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Restaurants.create(data);

    // Create relational data and return the entry.
    return Restaurants.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Restaurants.associations.map(a => a.alias));
    const data = _.omit(values, Restaurants.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Restaurants.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Restaurants.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Restaurants.attributes).reduce((acc, curr) => {
      switch (Restaurants.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
